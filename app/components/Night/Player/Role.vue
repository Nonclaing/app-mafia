    ...
    <div class="flex flex-col gap-1 items-center justify-center">
      ...
      <template v-else-if="!isEmpty(currentData) && get(currentData, 'check')">
        <template v-if="includes(get(currentData, 'roles'), player.role.id)">
          ...
          <img v-if="player.state.kick" class="w-[60px] h-[60px] object-contain" :src="useAssetUrl('/images/kick.png')">
        ...
        <template v-else>
          ...
          <img v-if="player.state.kick" class="w-[60px] h-[60px] object-contain" :src="useAssetUrl('/images/kick.png')">
          <img v-else class="w-[60px] h-[60px] object-contain" :src="useAssetUrl('/images/mafia.png')">
        </template>
      </template>
      <template v-else>
        <div class="font-bold mb-2">
        ...
        <img v-if="player.state.kick" class="w-[60px] h-[60px] object-contain" :src="useAssetUrl('/images/kick.png')">
        <img v-else-if="player.state.kill" class="w-[60px] h-[60px] object-contain rounded-full" :src="useAssetUrl('/images/dead.svg')">
        <img v-else class="w-[60px] h-[60px] object-contain" :src="useAssetUrl('/images/mafia.png')">
      </template>
    </div>
  </div>
